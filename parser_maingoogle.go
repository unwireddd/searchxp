...
func randomUserAgentG() string {
	...
	randNum := rand.Int() % len(userAgentsG)
	return userAgentsG[randNum]
}
...
func googleResultParsing(response *http.Response, rank int) ([]SearchResultG, error) {
...
func getScrapeClientG(proxyString interface{}) *http.Client {
...
func GoogleScrape(searchTerm, countryCode, languageCode string, proxyString interface{}, pages, count, backoff int) ([]SearchResultG, error) {
	results := []SearchResultG{}
	resultCounter := 0
	...
	for _, page := range googlePages {
		res, err := scrapeClientRequestG(page, proxyString)
		if err != nil {
			return nil, err
		}
...
func scrapeClientRequestG(searchURL string, proxyString interface{}) (*http.Response, error) {
	baseClient := getScrapeClientG(proxyString)
	req, _ := http.NewRequest("GET", searchURL, nil)
	req.Header.Set("User-Agent", randomUserAgentG())
